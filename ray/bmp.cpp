...
int main(void)
{
  ...
  image = new unsigned char [size = xw * yw * 3];
  if (image == 0) {
    ...
    std::ofstream bitmap(filename, std::ios::binary);
    if (bitmap == 0) {
...
*/
      ...
      bitmap.write((char *)&header, sizeof header);
      bitmap.write((char *)image, size);
